%{
Kenneth Lin
Noisy channel w/ equalization for cosim (Behavioral vs RTL)
%}
...
%{
%% STEP 3.5: dumping noisy waveform for RTL co-sim
...
fid = fopen('eq_output_dump.hex', 'r');
hex_data = textscan(fid, '%s');
fclose(fid);

% Converting to signed integer + voltage scaling
rtl_output_int = hex2dec(hex_data{1});
rtl_output_int(rtl_output_int > 32767) = rtl_output_int(rtl_output_int > 32767) - 65536;
rtl_waveform = rtl_output_int * lsb_to_volt;

% 1. Discard the first 300 samples to let the filters/adaptation settle
settled_tx     = tx_waveform(301:end); 
settled_rx     = rx_waveform_noisy(301:end); 
settled_eq     = equalized_waveform(301:end);
settled_eq_rtl = rtl_waveform(301:end);       % <-- Fixed: Slices directly from raw conversion vector

upsample_factor = 8;
samples_per_trace = upsample_factor * 2; % 2 Unit Intervals wide

% 2. Interpolate once to create smooth oscilloscope traces
...
eq_rtl_smooth   = interp(settled_eq_rtl, upsample_factor); % <-- Fixed: Clean single upsampling

% 3. Strip away the interpolation filter group delay
...
eq_rtl_smooth = eq_rtl_smooth(interp_delay_offset:end);

% 4. Calculate how many full 2-UI traces we can draw safely
% (Protects against any subtle length mismatches after delay stripping)
min_len = min([length(tx_smooth), length(rx_smooth), length(eq_smooth), length(eq_rtl_smooth)]);
num_traces = floor(min_len / samples_per_trace) - 1;
t_vector = (0:samples_per_trace-1) / upsample_factor;

% 5. Reshape long 1D arrays into 2D matrices for overlapping plots
...
eq_rtl_eye_matrix = reshape(eq_rtl_smooth(1:num_traces*samples_per_trace), samples_per_trace, num_traces);

% 6. Plotting bounds definition
num_traces_to_plot = min(200, num_traces);

% --- Plot Generation ---

% Ideal TX Eye
figure('Name', 'TX Eye', 'Position', [100, 100, 500, 400]);
plot(t_vector, tx_eye_matrix(:, 1:num_traces_to_plot), 'g', 'LineWidth', 0.5);
grid on; title('Ideal TX PAM4 Eye'); xlabel('Symbol Time (UI)'); ylabel('Voltage (V)'); ylim([-5 5]);

% Rx eye before equalization
figure('Name', 'RX Before EQ', 'Position', [300, 100, 500, 400]);
plot(t_vector, rx_eye_matrix(:, 1:num_traces_to_plot), 'r', 'LineWidth', 0.5);
grid on; title('Rx Input Before Equalization'); xlabel('Symbol Time (UI)'); ylabel('Voltage (V)'); ylim([-5 5]);

% Behavioral Equalized eye after FFE/DFE
figure('Name', 'RX After EQ (Behavioral)', 'Position', [500, 100, 500, 400]);
plot(t_vector, eq_eye_matrix(:, 1:num_traces_to_plot), 'b', 'LineWidth', 0.5);
grid on; title('After SS-LMS Equalization (Behavioral FFE + DFE)'); xlabel('Symbol Time (UI)'); ylabel('Voltage (V)'); ylim([-5 5]);

% RTL Equalized eye, RTL cosim
figure('Name', 'RTL RX After EQ', 'Position', [700, 500, 400, 300]);
plot(t_vector, eq_rtl_eye_matrix(:, 1:num_traces_to_plot), 'm', 'LineWidth', 0.5);
grid on; title('Verilog RTL (Hardware FFE + DFE)'); xlabel('Symbol Time (UI)'); ylabel('Voltage (V)'); ylim([-5 5]);

% Diagnostic Time-Domain Plot
figure('Name', 'Co-Sim Waveform Debugger');
plot(equalized_waveform(301:400), 'b', 'LineWidth', 2); hold on;
plot(rtl_waveform(301:400), 'm--', 'LineWidth', 1.5);
grid on;
legend('MATLAB Golden Behavioral', 'Verilog RTL');
title('Raw Equalizer Output Comparison (First 100 Settled Samples)');
xlabel('Sample Index');
ylabel('Voltage (V)');